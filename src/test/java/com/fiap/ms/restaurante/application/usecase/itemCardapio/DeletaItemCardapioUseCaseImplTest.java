...
public class DeletaItemCardapioUseCaseImplTest {
    ...
    void deletaItemCardapio_sucesso(){
        ...
        when(itemCardapioDomainService.buscarItemCardapioPorId(1L)).thenReturn(item);
        doNothing().when(itemCardapio).deletar(item);

        deletaItemCardapioUseCase.deletar(1L);

        verify(itemCardapioDomainService, times(1)).buscarItemCardapioPorId(1L);
